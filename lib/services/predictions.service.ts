...
/** Classement général : UNIQUEMENT les membres ayant fait au moins 1 pronostic */
export async function getGeneralStandings(limit = 50): Promise<StandingRow[]> {
  return safeQuery(async () => {
    const supabase = createSupabaseServerClient();

    // RPC dédiée (migration 020) : joueurs réels + nombre de pronostics
    const { data: rpc, error: rpcErr } = await supabase.rpc("get_general_standings", { p_limit: limit });
    if (!rpcErr && rpc) {
      return (rpc as StandingRow[]).map((r) => ({
        user_id: r.user_id,
        username: r.username,
        avatar_url: r.avatar_url,
        points: Number(r.points ?? 0),
        preds: Number(r.preds ?? 0),
      }));
    }

    // Fallback (migration pas encore jouée) : profils ayant marqué des points
    const { data } = await supabase
      .from("profiles")

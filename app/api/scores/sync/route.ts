...
import { syncLiveScores, importFixtures, syncStandings, cleanupPassedMatches, syncMatchEvents, backfillMissedResults, lastApiMeta, LIVE_API_STATUSES, resetSettleErrors, getSettleErrors } from "@/lib/services/football.service";
...
async function handle(req: Request) {
  ...
  try {
    ...
    // 4) Chaîne de fournisseurs : couche gratuite fusionnée (FD + ESPN +
    //    TheSportsDB + OpenLigaDB), api-football en dernier recours
    ...
    // FIXTURES D'ABORD (ESPN, sans clé, sans quota) : on remet les VRAIES
    // dates des matchs à venir et on règle les matchs finis. Les compteurs
    // "missed" redescendent naturellement ; le backfill api-football (quota
    // limité) ne reste qu'en dernier recours quand l'import n'a pas pu tourner.
    const lastFixtures =
      settings.sync_state.last_fixtures_import_espn ?? settings.sync_state.last_fixtures_import;
    const fixturesDue =
      now - lastFixtures > 6 * 3600_000 ||
      (missedSettlements > 0 && now - lastFixtures > 30 * 60_000);
    let fixtures: Awaited<ReturnType<typeof importFixtures>> | null = null;
    let fixturesOk = false;
    if (fixturesDue) {
      fixtures = await importFixtures();
      fixturesOk = fixtures !== null && fixtures.skipped === undefined;
      await updateSetting("sync_state", {
        last_fixtures_import: now,
        last_fixtures_import_espn: now,
      }).catch(() => {});
    }
    const backfillDue =
      missedSettlements > 0 && !fixturesOk && now - lastFixtures > 30 * 60_000;
    ...
    const debug = {
      ...
      fixturesReshaped: fixtures?.reshaped ?? 0,
    ...
    return NextResponse.json({
      ok: true,
      data: { ...result, eventsSynced: events.events, fixturesImported: fixtures?.imported ?? 0, fixturesReshaped: fixtures?.reshaped ?? 0, standingsUpdated: standings?.updated ?? 0, jobsUpserted: jobs?.upserted ?? 0 },
      cleaned: cleanupDue,
